public class reloj_movimiento {
    ...
    public static void main(String[] args) throws InterruptedException {
        int altura = Integer.parseInt(System.console().readLine("Introduzca la altura del reloj de arena (impar): "));
        ...
        int contador = 0;
        int der2 = -10; // Empieza en -10 porque no lo quiero usar hasta que llegue el contador a 8
        int izq2 = -10; // "         "       "
        ...
        while (fila2 >= mitad-2 ) {
            for (int i = 1; i < altura - 1; i++) {
                System.out.printf((espacios1 > 0) ? (asteriscos1 == 1)? "%" + (espacios1-1) + "s" :"%" + espacios1 + "s" : "", " "); //Espacios por delante de asteriscos
                for (int j = 0; j < asteriscos1; j++) {
                    if (asteriscos1 == 1) {
                        System.out.print("*");
                        System.out.print((fila2 >= mitad)? PURPLE+"*"+RESET:GREEN + "*" + RESET);
                        System.out.print("*");
                    }
                    
                    else if ((j == 0 && asteriscos1 != 1) || i == 1 || j == asteriscos1 - 1 || i == altura - 2 ) { // Asteriscos del exterior
                        System.out.print("*");
                        ...
                        if (j <= der2 && j >= izq2 && i == fila2 ) {
                            System.out.print(GREEN + "*" + RESET);
                        } else if ((j == i - (mitad) && i - (mitad) <= contador)) {
                ...
                } else if (i >= mitad) { // mitad reloj abajo
                    asteriscos1 += 2;
                    espacios1--;
